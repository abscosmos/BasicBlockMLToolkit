...
impl TraceData {
    pub fn summary(&self) -> TraceSummary {
        let (targeted, unique_apps) = if !self.filter {
            ...
            (Some(BlockCountStats::from_iter(iter)), Some(unique_ct(apps)))
        } else {
            (None, None)
        };
        ...
        TraceSummary {
            ...
            counts: BlockCountStats::from_iter(self.blocks.values()),
            targeted,
            unique_apps,
        }
    }
...
impl BlockCountStats {
    pub fn from_iter<'a>(iter: impl IntoIterator<Item=&'a BasicBlock, IntoIter: Clone>) -> Self {
